...
func TestEnvconfigProcess(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			name:     "defaults",
			input:    &publish.Config{},
			exp:      publish.TestConfigDefaults(),
			lookuper: envconfig.MapLookuper(map[string]string{}),
		},
		{
			name:     "values",
			input:    &publish.Config{},
			exp:      publish.TestConfigValued(),
			lookuper: envconfig.MapLookuper(publish.TestConfigValues()),
		},
		{
			name:     "overrides",
			input:    publish.TestConfigOverridden(),
			exp:      publish.TestConfigOverridden(),
			lookuper: envconfig.MapLookuper(publish.TestConfigValues()),
		},
	}
