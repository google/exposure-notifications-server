...
func getFederationSync(ctx context.Context, syncID string, queryRow queryRowFn) (*model.FederationSync, error) {
	row := queryRow(ctx, `
		SELECT
...
func (db *DB) StartFederationSync(ctx context.Context, q *model.FederationQuery, started time.Time) (string, FinalizeSyncFn, error) {
	...
	syncID := uuid.New().String() // TODO(jasonco): other tables use sequence, not uuid.
	_, err = conn.Exec(ctx, `
		INSERT INTO
			FederationSync
