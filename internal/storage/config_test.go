...
func TestEnvconfigProcess(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			name:     "defaults",
			input:    &storage.Config{},
			exp:      storage.TestConfigDefaults(),
			lookuper: envconfig.MapLookuper(map[string]string{}),
		},
		{
			name:     "values",
			input:    &storage.Config{},
			exp:      storage.TestConfigValued(),
			lookuper: envconfig.MapLookuper(storage.TestConfigValues()),
		},
		{
			name:     "overrides",
			input:    storage.TestConfigOverridden(),
			exp:      storage.TestConfigOverridden(),
			lookuper: envconfig.MapLookuper(storage.TestConfigValues()),
		},
	}
