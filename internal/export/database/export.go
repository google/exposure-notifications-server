...
func (db *ExportDB) AddExportConfig(ctx context.Context, ec *model.ExportConfig) error {
	...
	return db.db.InTx(ctx, pgx.Serializable, func(tx pgx.Tx) error {
		row := tx.QueryRow(ctx, `
			INSERT INTO
				ExportConfig
				(bucket_name, filename_root, period_seconds, region, from_timestamp, thru_timestamp, signature_info_ids, input_regions)
			VALUES
				($1, $2, $3, $4, $5, $6, $7, $8)
			RETURNING config_id
	...
	return db.db.InTx(ctx, pgx.Serializable, func(tx pgx.Tx) error {
		result, err := tx.Exec(ctx, `
			UPDATE
				ExportConfig
			SET
				bucket_name = $1, filename_root = $2, period_seconds = $3, region = $4, from_timestamp = $5, thru_timestamp = $6, signature_info_ids = $7, input_regions = $8
			WHERE config_id = $9
		`, ec.BucketName, ec.FilenameRoot, int(ec.Period.Seconds()), ec.Region,
...
func (db *ExportDB) GetExportConfig(ctx context.Context, id int64) (*model.ExportConfig, error) {
	...
	row := conn.QueryRow(ctx, `
		SELECT
			config_id, bucket_name, filename_root, period_seconds, region, from_timestamp, thru_timestamp, signature_info_ids, input_regions
		FROM
			ExportConfig
		WHERE
			config_id = $1`, id)
...
func scanOneExportConfig(row pgx.Row) (*model.ExportConfig, error) {
	...
	if err := row.Scan(&m.ConfigID, &m.BucketName, &m.FilenameRoot, &periodSeconds, &m.Region, &m.From, &thru, &m.SignatureInfoIDs, &m.InputRegions); err != nil {
		return nil, err
	}
...
func (db *ExportDB) AddExportBatches(ctx context.Context, batches []*model.ExportBatch) error {
	return db.db.InTx(ctx, pgx.Serializable, func(tx pgx.Tx) error {
		...
		_, err := tx.Prepare(ctx, stmtName, `
			INSERT INTO
				ExportBatch
				(config_id, bucket_name, filename_root, start_timestamp, end_timestamp, region, status, signature_info_ids, input_regions)
			VALUES
				($1, $2, $3, $4, $5, $6, $7, $8, $9)
		`)
		if err != nil {
		...
		for _, eb := range batches {
			if _, err := tx.Exec(ctx, stmtName,
				eb.ConfigID, eb.BucketName, eb.FilenameRoot, eb.StartTimestamp, eb.EndTimestamp, eb.Region, eb.Status, eb.SignatureInfoIDs, eb.InputRegions); err != nil {
				return err
			}
		}
		return nil
...
func lookupExportBatch(ctx context.Context, batchID int64, queryRow queryRowFn) (*model.ExportBatch, error) {
	row := queryRow(ctx, `
		SELECT
			batch_id, config_id, bucket_name, filename_root, start_timestamp, end_timestamp, region, status, lease_expires, signature_info_ids, input_regions
		FROM
			ExportBatch
		WHERE
			batch_id = $1
	...
	if err := row.Scan(&eb.BatchID, &eb.ConfigID, &eb.BucketName, &eb.FilenameRoot, &eb.StartTimestamp, &eb.EndTimestamp, &eb.Region, &eb.Status, &expires, &eb.SignatureInfoIDs, &eb.InputRegions); err != nil {
		if err == pgx.ErrNoRows {
...
func (db *ExportDB) LookupExportFile(ctx context.Context, filename string) (*model.ExportFile, error) {
	...
	row := conn.QueryRow(ctx, `
		SELECT
			bucket_name, filename, batch_id, region, batch_num, batch_size, status, input_regions
		FROM
			ExportFile
		WHERE
			filename = $1
	...
	ef := model.ExportFile{}
	if err := row.Scan(&ef.BucketName, &ef.Filename, &ef.BatchID, &ef.Region, &ef.BatchNum, &ef.BatchSize, &ef.Status, &ef.InputRegions); err != nil {
		if err == pgx.ErrNoRows {
			return nil, database.ErrNotFound
		}
		return nil, err
...
func addExportFile(ctx context.Context, tx pgx.Tx, ef *model.ExportFile) error {
	tag, err := tx.Exec(ctx, `
		...
		`, ef.BucketName, ef.Filename, ef.BatchID, ef.Region, ef.BatchNum, ef.BatchSize, ef.Status, ef.InputRegions)
	if err != nil {
