...
// Process2 processes the environment into the given interface. It is the same
// as calling `ProcessWith(i, new(OsLookuper))`.
func Process2(i interface{}) error {
	return ProcessWith(i, new(OsLookuper))
}
...
func ProcessWith(i interface{}, l Lookuper) error {
	if l == nil {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if ef.Kind() == reflect.Struct {
			...
			if err := ProcessWith(ef.Interface(), l); err != nil {
				return fmt.Errorf("%s: %w", tf.Name, err)
			}
