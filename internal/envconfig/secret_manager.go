...
// SecretManagerMutatorFunc returns a function that fetches secrets from the
// secret manager. If the provided secret manager is nil, the function is nil,
// Otherwise, it looks for values prefixed with secret:// and resolves them as
// secrets.
func SecretManagerMutatorFunc(sm secrets.SecretManager, smConfig *secrets.Config) MutatorFunc {
	if sm == nil {
		return nil
	}
