...
type (
	...
	insertExposuresFn     func(context.Context, []*publishmodel.Exposure, *pb.RevisionTokenData, bool, bool) (int, error)
			...
			for _, cti := range ctr.ContactTracingInfo {
				for _, key := range cti.ExposureKeys {
					...
					if len(exposures) == fetchBatchSize {
						n := 0
						if n, err = opts.deps.insertExposures(ctx, exposures, nil, false, false); err != nil {
