...
func (aa *AuthorizedAppDB) InsertAuthorizedApp(ctx context.Context, m *model.AuthorizedApp) error {
	...
	return aa.db.InTx(ctx, pgx.Serializable, func(tx pgx.Tx) error {
		result, err := tx.Exec(ctx, `
			INSERT INTO
				AuthorizedApp
			  (app_package_name, platform, allowed_regions, health_authority_ids,
		...
		`, m.AppPackageName, m.Platform, m.AllAllowedRegions(), m.AllAllowedHealthAuthorities(),
...
func (aa *AuthorizedAppDB) UpdateAuthorizedApp(ctx context.Context, priorKey string, m *model.AuthorizedApp) error {
	return aa.db.InTx(ctx, pgx.Serializable, func(tx pgx.Tx) error {
		result, err := tx.Exec(ctx, `
			...
			SET
				app_package_name = $1, platform = $2, allowed_regions = $3, health_authority_ids = $4,
...
func scanOneAuthorizedApp(ctx context.Context, row pgx.Row, sm secrets.SecretManager) (*model.AuthorizedApp, error) {
	...
	var allowedHealthAuthorities []int64
	...
	if err := row.Scan(
		&config.AppPackageName, &config.Platform, &allowedRegions, &allowedHealthAuthorities,
	...
	for _, haID := range allowedHealthAuthorities {
		config.AllowedHealthAuthorities[haID] = struct{}{}
	}
