...
func (s *Server) handleMirror(ctx context.Context) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, mirror := range mirrors {
			...
			curFiles, err := s.downloadIndex(mirror)
			if err != nil {
				logger.Errorw("unable to download index.txt", "error", err)
			}
			...
			for filename, status := range actions {
				if status.needsDelete() {
					...
					bsCtx, cancel := context.WithTimeout(ctx, time.Second*30)
					defer cancel()
			...
			logger.Infow("mirror state", "mirrorID", mirror.ID, "total", len(actions), "existing", len(indexObjects), "todo", len(actions)-len(indexObjects))
