...
var (
	url                  = flag.String("url", "http://localhost:8080/publish", "http(s) destination to send test record")
...
func TestIntegrationUtil(t *testing.T) {
	...
	db, _ := StartSystemUnderTest(t, ctx)

	authApp := aamodel.NewAuthorizedApp()
	authApp.AppPackageName = *appPackage
	authApp.AllowedRegions["US"] = struct{}{}
	appDB := aadb.New(db)

	appDB.InsertAuthorizedApp(ctx, authApp)
